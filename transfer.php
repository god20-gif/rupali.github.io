...
$username="root";
$password="";
$con=mysqli_connect($server,$username,$password,"rups");
if(!$con){
...
$sql = "SELECT balance FROM rups WHERE name='$sender'";
...
if ($result->num_rows > 0) {
 ...
 if($amount>$row["balance"] or $row["balance"]-$amount<100){
  ...
  header("Location: $location&message=transactionDenied");
    ...
    $sql = "UPDATE `rups` SET balance=(balance-$amount) WHERE name='$sender'";
...
if($flag==true){
$sql = "UPDATE `rups` SET balance=(balance+$amount) WHERE name='$receiver'";
...
if($flag==true){
    $sql = "SELECT * from rups where name='$sender'";
    $result = $con-> query($sql);
    while($row = $result->fetch_assoc())
     {
         $s_acc=$row['ac'];
...
//  Transcation DEatiled Stored in the DB
 ...
 $sql = "SELECT * from rups where name='$receiver'";
 $result = $con-> query($sql);
 while($row = $result->fetch_assoc())
  {
      $r_acc=$row['ac'];
